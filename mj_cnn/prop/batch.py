...
def _read_samples4tr(sent_filepath, con_filepath, ans_filepath, bos_id, eos_id):
    ret = []
    with open(sent_filepath) as sfp, open(con_filepath) as cfp, open(ans_filepath) as afp:
        for sent, con, ans in zip(sfp, cfp, afp):
            sent = sent.split("\n")[0]
            con = con.split("\n")[0]
            ans = ans.split("\n")[0]
            s = [bos_id] + [int(x) for x in sent.split()] + [eos_id]
            c = [bos_id] + [int(x) for x in con.split()] + [eos_id]
            a = float(ans)
            ret.append((s,c,a))
    return ret
...
def generate_samples(
        ...
        bos_id,
        eos_id):
    samples = _read_samples4tr(sent_filepath, con_filepath, ans_filepath, bos_id, eos_id)
    return samples
