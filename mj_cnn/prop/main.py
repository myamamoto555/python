...
train_con_file = './data/train.context' 
...
train_con_wid = './data/train_con.wid'
...
if __name__ == '__main__':
    # create vocab file
    util.make_vocab(train_con_file, voc_file, voc_size)
    # convert word to id
    util.apply_vocabs(train_con_file, train_con_wid, voc_file)
    ...
    # create samples
    train_samples = batch.generate_samples(train_wid, train_con_wid, train_ans_file, 
                                           BOS_ID, EOS_ID)
    ...
    for ep in range(epoch):
        ...
        for b in batches:
            with chainer.using_config('train', True):
                x_list, c_list, t_list = b
                mdl.zerograds()
                loss = mdl.train(x_list, c_list, t_list)
                loss.backward()
                opt.update()
